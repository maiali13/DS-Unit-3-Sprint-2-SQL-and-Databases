...
```
This is an example of an "one to many" relationship, because one employee might be associated with many territories. This also illustrated in the provided database schema, where you can see that the 'Employee' and 'Territory' tables are connected via the 'EmployeeTerritories' table. If employees were responsible for multiple territories, but also shared those territories with other employees, it would be a "many to many" relationship.
```

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
```
MongoDB is a NoSQL DB management program, meaning that it is excellent for storing complex data types in document format instead of in relational, rigid rows. So, MongoDB is an appropriate choice if you are running an application that contains large amount of diverse, changing data that and/or benefits from horizontal scaling.  However, it is not appropriate in applications which need to fully meet ACID standards, and/or might be extremely sensitive, like monetary transactions.
```


- What is "NewSQL", and what is it trying to achieve?
```
NewSQL are "new" DB management systems that attempt to bring the best features of both SQL and NoSQL together- AKA allowing for the more flexible database structure and horizontal scaling of NoSQL while also maintaining ACID standards to allow transactions to take place provided by traditional SQL.
```
